...
function convertJSONToTS(
  storyId: string,
  translations: Map<string, TranslationJSON>,
): string {
...
function generateTranslations() {
  ...
  for (const storyDir of storyDirs) {
    ...
    const translations = new Map<string, TranslationJSON>();
    ...
    for (const file of files) {
      ...
      const json: TranslationJSON = JSON.parse(content);
...
function updateIndexFile(storyIds: string[], fileNames: string[]) {
